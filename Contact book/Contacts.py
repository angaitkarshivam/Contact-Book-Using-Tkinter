...
class Contacts:
    ...
    def create_left_icon(self):
        photo = PhotoImage(file='E:\\Github\\Contact book\\Logogif.gif')
        label = Label(image=photo)
        label.image = photo
        label.grid(row=0, column=0)

    def create_label_frame(self):
        labelframe = LabelFrame(self.root, text='Create New Contact', bg='#002B66', fg='white', font=('helvetica 10'))
    ...
    def create_bottom_buttons(self):
        Button(text='Delete Selected', command=self.delete, bg='red', fg='white').grid(row=8, column=0, sticky=W, padx=20, pady=10)
        Button(text='Modify Selected', command=self.update, bg='purple', fg='white').grid(row=8, column=1, sticky=W)
    ...
    def update(self):
        ...
        try:
            ...
            self.message['text'] = 'No iten selected to modify'
            return
    ...
    def view_records(self):
        ...
        contact_enteries = self.execute_db_query(query)
        for row in contact_enteries:
